...
import {
  Target, TrendingUp, Star, ArrowRight, Cpu,
  Brain, Settings2, Network, Cloud,
} from "lucide-react";
...
const machines = [
  {
    ...
    title: <>Monolayer Blown Film Extrusion <br /> Line</>,
    specs: [
  ...
  return (
    <>
      ...
      <div className="w-full bg-[#F8F9FA] overflow-hidden">
        ...
        <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-10 lg:px-8 py-10 sm:py-14 md:py-16 flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-16">
          ...
          >
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="flex -space-x-3">
                {avatars.map((src, i) => (
                  <img key={i} src={src} alt="" className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border-2 border-white object-cover" />
                ))}
              </div>
              <div>
                <div className="flex items-center gap-1">
                  <Star className="w-4 h-4 fill-amber-400 text-amber-400" />
                  <span className="font-bold text-gray-800 text-sm">4.9/5 Rating</span>
                </div>
                <p className="text-gray-400 text-xs">Trusted by 10k+ industry experts</p>
              </div>
            </div>
          </div>
          ...
          >
            ...
            >
              ...
              <div>
                <p className="text-xs text-[#404945] font-semibold">Precision</p>
                <p className="text-sm sm:text-lg font-extrabold text-[#003429]">100%</p>
              </div>
            </div>
            ...
            >
              ...
              <div>
                <p className="text-xs text-[#404945] font-semibold">Production</p>
                <p className="text-sm sm:text-lg font-extrabold text-[#003429]">+40% Up</p>
              </div>
            </div>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div
              ...
              className={`flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8 sm:mb-10 transition-all duration-700 ${
                machineHeadingInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
              }`}
            >
              <div>
              ...
              {machines.map((machine, i) => (
                <div
                  ...
                  className={`bg-white rounded-[12px] overflow-hidden flex flex-col transition-all duration-700 hover:-translate-y-1 hover:shadow-xl ${
                    machineCardsInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                  }`}
                  style={{ transitionDelay: machineCardsInView ? `${i * 120}ms` : "0ms" }}
                >
                  ...
                  <div className="p-5 sm:p-6 flex flex-col flex-1">
                    ...
                    <div className="flex flex-col gap-3 flex-1">
                      {machine.specs.map((spec, j) => (
                        ...
                        >
                          <span className="text-[#404945] font-medium text-sm">{spec.label}</span>
                          <span className="text-[#191C1D] font-semibold text-sm">{spec.value}</span>
                        </div>
                      ))}
                    </div>
                    <a
        ...
        {/* ── Features ── */}
        <section className="py-14 sm:py-8 md:py-10 px-4 sm:px-6 md:px-10 lg:px-8">
          <div className="max-w-7xl mx-auto">

            <div
              ref={featHeadingRef}
              className={`text-center mb-10 sm:mb-14 transition-all duration-700 ${
                featHeadingInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
              }`}
            >
                ...
                return (
                  <div
                    ...
                    className={`bg-white rounded-2xl p-6 sm:p-7 flex flex-col gap-4 transition-all duration-700 hover:-translate-y-1 hover:shadow-lg ${
                      featCardsInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                    }`}
                    style={{ transitionDelay: featCardsInView ? `${i * 100}ms` : "0ms" }}
                  >
                    <Icon className="w-8 h-8 text-[#004D3D]" strokeWidth={1.5} />
                    <div>
                      <h3 className="text-base sm:text-lg font-bold text-[#003429] mb-2">{f.title}</h3>
                      <p className="text-[#003429] font-medium leading-relaxed">{f.description}</p>
                    </div>
                  </div>
                );
              })}
